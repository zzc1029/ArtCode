...
static void xINT_BCAN(CAN_SFRmap* CANx)
{
 ...
 CAN_Set_INT_Enable(CANx,((uint32_t)1<<0)|((uint32_t)1<<2),TRUE);
...
{
  xGPIO_Bcan();
  xInit_CAN(((CAN_SFRmap *) ((uint32_t)0x40001980)),CAN_BAUDRATE_125K,((uint32_t)0<<(1)));
  xINT_BCAN(((CAN_SFRmap *) ((uint32_t)0x40001980)));
}
...
void InitBCanSend(void)
{
    Bcan_fifo_init(&BCanSendBuff,BCanSendbuffx,((1<<(6))));
...
uint32_t BCan_Standard_Tx_Bytes(uint16_t id,uint8_t * buffer,uint8_t len)
{
 ...
 if(Art.bit.b30)
  return 0;
    temp = (BCanSendBuff.max_len - BCanSendBuff.wr_index + BCanSendBuff.rd_index);
 if(1 > temp)
    {
 ...
 BCanSendBuff.buff[temp].type = ((uint32_t)0);
...
uint32_t BCan_Expend_Tx_Bytes(uint32_t id,uint8_t * buffer,uint8_t len)
{
 ...
 BCanSendBuff.buff[temp].ID = id;
 BCanSendBuff.buff[temp].len = len;
 BCanSendBuff.buff[temp].type = ((uint32_t)1);
 BCanSendBuff.buff[temp].sended = SENDED;
 BCanSendBuff.wr_index += 1;
 return 1;
...
uint32_t BCanSendFifo(void)
{
    ...
    temp = (BCanSendBuff.wr_index - BCanSendBuff.rd_index);
    if(temp < 1)
    {
        return 0;
    }
...
void BCanSendTo(void)
{
   ...
   BcanDelay = 10;
   if(BCandomsg.type == ((uint32_t)0))
    CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40001980)),BCandomsg.ID,BCandomsg.data,BCandomsg.len,((uint32_t)0),((uint32_t)0));
   else
    CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40001980)),BCandomsg.ID,BCandomsg.data,BCandomsg.len,((uint32_t)0),((uint32_t)1));
     }
     break;
     case DELAYING:
     if((BcanDelay == 0) || (CAN_Get_Transmit_Status(((CAN_SFRmap *) ((uint32_t)0x40001980)),((uint32_t)((uint32_t)1<<((19)))))))
     {
     BCandomsg.sended = SENDED;
   BcanDelay = 0;
