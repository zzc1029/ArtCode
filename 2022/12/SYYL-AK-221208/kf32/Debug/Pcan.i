...
# 40 "../driver/Pcan.c"
 ...
 void InitPCanSend(void)
 {
  Pcan_fifo_init(&PCanSendBuff,PCanSendbuffx,((1<<(6))));
 ...
 uint32_t PCan_Standard_Tx_Bytes(uint16_t id,uint8_t * buffer,uint8_t len)
 {
  ...
  temp = (PCanSendBuff.max_len - PCanSendBuff.wr_index + PCanSendBuff.rd_index);
  if(1 > temp)
  {
  ...
  PCanSendBuff.buff[temp].type = ((uint32_t)0);
 ...
 uint32_t PCan_Expend_Tx_Bytes(uint32_t id,uint8_t * buffer,uint8_t len)
 {
  ...
  PCanSendBuff.buff[temp].ID = id;
  PCanSendBuff.buff[temp].len = len;
  PCanSendBuff.buff[temp].type = ((uint32_t)1);
  PCanSendBuff.buff[temp].sended = SENDED;
  PCanSendBuff.wr_index += 1;
  return 1;
 ...
 uint32_t PCanSendFifo(void)
 {
  ...
  temp = (PCanSendBuff.wr_index - PCanSendBuff.rd_index);
  if(temp < 1)
  {
   return 0;
  }
 ...
 switch(PCandomsg.sended)
 {
   case SENDED:
    ...
    PcanDelay = 10;
    if(PCandomsg.type == ((uint32_t)0))
     CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40001180)),PCandomsg.ID,PCandomsg.data,PCandomsg.len,((uint32_t)0),((uint32_t)0));
    else
     CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40001180)),PCandomsg.ID,PCandomsg.data,PCandomsg.len,((uint32_t)0),((uint32_t)1));
     }
     break;
   case DELAYING:
     if((PcanDelay == 0) || (CAN_Get_Transmit_Status(((CAN_SFRmap *) ((uint32_t)0x40001180)),((uint32_t)((uint32_t)1<<((18)))))))
     {
    PCandomsg.sended = SENDED;
