...
void AdcInit(void)
{
	...
	ADC_Regular_Channel_Config (ADC0_SFR, ADC_CHANNEL_20, 0x01); //ADC0常规通道1的采样源设置为ADC_CHANNEL_1
...
void  ReadAdcResult(void)
{
    ...
    switch(flag)
    {
      case metera2:
		    buf_16 = buf_16<<1;
	  	   #if(A2_TYE == RES)
		    meter.MeterA2 = buf_16*(VREF/4096.0);
		    meter.MeterA2 = (A2_PULL*meter.MeterA2)/(5000 - meter.MeterA2);
		    //meter.MeterA2 = (A2_PULL*buf_16)/(4096 - buf_16);
			#elif(A2_TYE == VOLT)
			buf_16 = buf_16<<1;
			meter.MeterA2 = buf_16*(VREF/4096.0);
			#endif
	  ...
	  case metera3:
		    buf_16 = buf_16<<1;
	  	   #if(A3_TYE == RES)
		   	meter.MeterA3 = buf_16*(VREF/4096.0);
		    meter.MeterA3 = (A3_PULL*meter.MeterA3)/(5000 - meter.MeterA3);
		    //meter.MeterA3 = (A3_PULL*buf_16)/(4096 - buf_16);
			#elif(A3_TYE == VOLT)
			buf_16 = buf_16<<1;
			meter.MeterA3 = buf_16*(VREF/4096.0);
			#endif	 
	  ...
	  case metera4:
	  	   buf_16 = buf_16<<1;
	  	   #if(A4_TYE == RES)
		    meter.MeterA4 = buf_16*(VREF/4096.0);
		    meter.MeterA4 = (A4_PULL*meter.MeterA4)/(5000 - meter.MeterA4);
		    //meter.MeterA4 = (A4_PULL*buf_16)/(4096 - buf_16);
			#elif(A4_TYE == VOLT)
			buf_16 = buf_16<<1;
			meter.MeterA4 = buf_16*(VREF/4096.0);
			#endif	
	  ...
	  case metera13:
	  	   buf_16 = buf_16<<1;
	  	   #if(A13_TYE == RES)
		   meter.MeterA13 = buf_16*(VREF/4096.0);
		   meter.MeterA13 = (A13_PULL*meter.MeterA13)/(5000 - meter.MeterA13);
		    //meter.MeterA13 = (A13_PULL*buf_16)/(4096 - buf_16);
			#elif(A13_TYE == VOLT)
			buf_16 = buf_16<<1;
			meter.MeterA13 = buf_16*(VREF/4096.0);
			#endif
	  ...
	  case metera14:
	  	   ...
	  	   #if(A14_TYE == RES)
		   meter.MeterA14 = buf_16*(VREF/4096.0);
		   meter.MeterA14 = (A14_PULL*meter.MeterA14)/(5000 - meter.MeterA14);
		   // meter.MeterA14 = (A14_PULL*(uint32_t)buf_16)/(4096 - buf_16);
			#elif(A14_TYE == VOLT)
			buf_16 = buf_16<<1;
			meter.MeterA14 = buf_16*(VREF/4096.0);
			#endif	
	  ...
	  case metervbat:
			...
			Vbat = Vbat/100;
		    Filter(Vbat);
	  	    ADC_Regular_Channel_Config (ADC0_SFR, ADC_CHANNEL_3, 0x01);
	  	    flag = meterE3E8; 
	  	    break;	 
	  case meterE3E8:
	  	    if(e3e8state == E3)
	  	    {
			  adc_100ma_fd[0] = buf_16*(VREF/4096.0)/100;
			...
			{
				adc_100ma_fd[3] = buf_16*(VREF/4096.0)/100;
				adc_100ma_fd[3] = (uint16_t)adc_100ma_fd[3] * 2250/1200;
				Filter(adc_100ma_fd[3]);
				e3e8state = E3;
			}
			SEL_E3E8(e3e8state);
	  	    ADC_Regular_Channel_Config (ADC0_SFR, ADC_CHANNEL_1, 0x01);
	  	    flag = meterE6E7; 	  	
	  	   break;
	  case meterE6E7:
	  	    if(e6e7state == E6)
	  	    {
			  adc_100ma_fd[1] = buf_16*(VREF/4096.0)/100;
			...
			{
				adc_100ma_fd[2] = buf_16*(VREF/4096.0)/100;
				adc_100ma_fd[2] = (uint16_t)adc_100ma_fd[2] * 2250/1200;
				Filter(adc_100ma_fd[2]);
				e6e7state = E6;
			}
			SEL_E6E7(e6e7state);
	  	    ADC_Regular_Channel_Config (ADC0_SFR, ADC_CHANNEL_2, 0x01);
	  	    flag = meterA16; 	  	
	  	   break;
	   case meterA16:
			adc_100ma_fd[4] = buf_16*(VREF/4096.0);
			adc_100ma_fd[4] = (uint16_t)adc_100ma_fd[4] * 2250/1200;
		    Filter(adc_100ma_fd[4]);
	  	    ADC_Regular_Channel_Config (ADC0_SFR, ADC_CHANNEL_9, 0x01);
	  	    flag = meterA15_NEW; 	  	
	  	   break;	
	  case meterA15_NEW:
			//buf_16 = buf_16 >>1;
			buf_16 = buf_16 * 3;
		    meter.MeterA15 = buf_16*(VREF/4096.0);
		    meter.MeterA15 = (A15_PULL*meter.MeterA15)/(5000 - meter.MeterA15);
			Filter(meter.MeterA15);
