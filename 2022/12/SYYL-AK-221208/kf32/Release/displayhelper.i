...
# 1 "D:/zzc/project/pro_2022/pro_12/SYYL-AK-221208/kf32/driver/keyfifo.h" 1
# 12 "D:/zzc/project/pro_2022/pro_12/SYYL-AK-221208/kf32/driver/keyfifo.h"
...
static u32 MainMesg(u8 key,u8 initpw)
{
 ...
 if(!port_c.bit.b4)
 {
# 229 "../source/displayhelper.c"
  return DIS_NOP_MENU;
 }
 else if(0 && port_c.bit.b4)
...
static u8 ChangePulse(u8 key,u8 initpw)
{
 ...
 static u16 set_Pulse_last;
  ...
  if(kehu_mi)
  {
   ...
   if((Set_Pulse > (1.2*1750))||(Set_Pulse < (0.8*1750)))
   {
...
static u8 ChangeOilAlarm(u8 key,u8 initpw)
{
 ...
 static u8 set_oilalarm_last = 0;
  ...
  if(kehu_mi)
  {
   ...
   if((Set_Oil_Alarm > (1.2*105))||(Set_Oil_Alarm < (0.8*105)))
   {
...
static u32 MenuDIS_POWER_MENU(u8 key,u8 initpw)
{
 ...
 if((!(0 || 0))||port_c.bit.b4||(Ankai_Add_190703.bit.b5 == 1))
  return DIS_NOP_MENU;
 else
  return DIS_POWER_MENU;
}
# 2702 "../source/displayhelper.c"
static u32 NopMesg(u8 key,u8 initpw)
{
   ...
   case 3:
    ...
    else if((0 || 0) && (Ankai_Add_190703.bit.b5 != 1))
    {
       flag = 0;
     return DIS_POWER_MENU;
    }
...
void Meter_Add_Menu(u8 initpw)
{
 ...
 communicate_T123_data[6] = Temp_Engine;
 communicate_T123_data[7] = Temp_Jiareqi;
    do{ u8 change = 0; for(i=3; i<11; i++) { if(data1save[i] != communicate_T123_data[i]) { data1save[i] = communicate_T123_data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); } }while(0);
}
...
void roll_alarm_Task(void)
{
    if((port_c.bit.b4&&(Art.bit.b28)) || 0)
    {
        roll_alarm();
    }
}
...
# 3554 "../source/displayhelper.c"
 if((0 || 0)||port_c.bit.b4||(Ankai_Add_190703.bit.b5 == 0))
  return DIS_NOP_MENU;
 else
  return DIS_PARK_MENU;
...
void SendPAGE_MAIN04(u8 initpw)
{
 ...
 u8 i; static u8 data1save[11]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; for(i = 1; i < 12; i++) communicate_T123_data[i] = 0;;
    ...
    communicate_T123_data[3] = (((scr)>>0)&0xFF);
 communicate_T123_data[4] = (((scr)>>8)&0xFF);
 do{ u8 change = 0; for(i=3; i<11; i++) { if(data1save[i] != communicate_T123_data[i]) { data1save[i] = communicate_T123_data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); } }while(0);
