...
void AdcPortInit(void)
{
   ...
   GPIO_Write_Mode_Bits(((GPIO_SFRmap *) ((uint32_t)0x50000080)),((uint16_t)0x0200),GPIO_MODE_AN);
}
...
void ReadAdcResult(void)
{
 ...
 buf_16 = ADC_Get_Conversion_Value(((ADC_SFRmap *) ((uint32_t)0x40000580)));
   ...
   case metera3:
      ...
      meter.MeterA3 = (1000*meter.MeterA3)/(5000 - meter.MeterA3);
   ...
   case metera4:
      ...
      meter.MeterA4 = (1000*meter.MeterA4)/(5000 - meter.MeterA4);
   ...
   Vbat = Vbat/100;
      { static uint8_t aver_count = 0,filter_count = 0; static uint16_t log[16]; static uint32_t sum; if (filter_count == 0) { sum = (Vbat); log[0] = (Vbat); filter_count ++; } else { if (filter_count < 16) { sum += (Vbat); log[filter_count] = (Vbat); Vbat = sum / (filter_count + 1); filter_count ++; } else { sum = sum - log[aver_count]; sum += (Vbat); log[aver_count] = (Vbat); Vbat = sum / 16; aver_count++; if (aver_count > 15) { aver_count = 0; } } } };
        ADC_Regular_Channel_Config (((ADC_SFRmap *) ((uint32_t)0x40000580)), ((uint32_t)0x00000003), 0x01);
        flag = meterE3E8;
        break;
   case meterE3E8:
        if(e3e8state == E3)
   ...
   adc_100ma_fd[4] = (uint16_t)adc_100ma_fd[4] * 2250/1200;
      { static uint8_t aver_count = 0,filter_count = 0; static uint16_t log[16]; static uint32_t sum; if (filter_count == 0) { sum = (adc_100ma_fd[4]); log[0] = (adc_100ma_fd[4]); filter_count ++; } else { if (filter_count < 16) { sum += (adc_100ma_fd[4]); log[filter_count] = (adc_100ma_fd[4]); adc_100ma_fd[4] = sum / (filter_count + 1); filter_count ++; } else { sum = sum - log[aver_count]; sum += (adc_100ma_fd[4]); log[aver_count] = (adc_100ma_fd[4]); adc_100ma_fd[4] = sum / 16; aver_count++; if (aver_count > 15) { aver_count = 0; } } } };
        ADC_Regular_Channel_Config (((ADC_SFRmap *) ((uint32_t)0x40000580)), ((uint32_t)0x00000009), 0x01);
        flag = meterA15_NEW;
       break;
   case meterA15_NEW:

   buf_16 = buf_16 * 3;
      meter.MeterA15 = buf_16*(3300/4096.0);
      meter.MeterA15 = (1000*meter.MeterA15)/(5000 - meter.MeterA15);
   { static uint8_t aver_count = 0,filter_count = 0; static uint16_t log[16]; static uint32_t sum; if (filter_count == 0) { sum = (meter.MeterA15); log[0] = (meter.MeterA15); filter_count ++; } else { if (filter_count < 16) { sum += (meter.MeterA15); log[filter_count] = (meter.MeterA15); meter.MeterA15 = sum / (filter_count + 1); filter_count ++; } else { sum = sum - log[aver_count]; sum += (meter.MeterA15); log[aver_count] = (meter.MeterA15); meter.MeterA15 = sum / 16; aver_count++; if (aver_count > 15) { aver_count = 0; } } } };
        ADC_Regular_Channel_Config (((ADC_SFRmap *) ((uint32_t)0x40000580)), ((uint32_t)0x00000014), 0x01);
        flag = metera2;
   break;
 }
