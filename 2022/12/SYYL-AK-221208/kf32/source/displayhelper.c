...
static u32 MenuDIS_POWER_MENU(u8 key,u8 initpw)  //松正
{
	...
	if((!(KEY_CHARGE || KEY_POWER))||KEY_ON||(Ankai_Add_190703.bit.b5 == 1))
		return DIS_NOP_MENU;
	else
		return DIS_POWER_MENU;
}
...
void meter_information(void)
{
    ...
    if((meterx&0x03)==0x01)
    {
        ...
        b2_add.bit.b5 = PTD1;
		//3.0版本使用
        b2_add.bit.b4 = PTC17;  //PTC17
        b2_add.bit.b3 = PTC16;  //PTC16
        b2_add.bit.b2 = PTC15;   
        b2_add.bit.b1 = PTC14;
        b2_add.bit.b0 = PTC13;
...
void Meter_Add_Menu(u8 initpw)
{
    ...
    outpower_state.bit.b4 = PTE2;
    outpower_state.bit.b3 = PTD16;
    outpower_state.bit.b2 = PTD15;
    outpower_state.bit.b1 = PTD14;
    outpower_state.bit.b0 = PTD13;
...
u32 MenuDIS_PARK_MENU(u8 key,u8 initpw)
{
   ...
   /*if(falg != Ankai_Add_190703.bit.b5)
   {
	...
	if((KEY_CHARGE || KEY_POWER)||KEY_ON||(Ankai_Add_190703.bit.b5 == 0))
		return DIS_NOP_MENU;
	else
		return DIS_PARK_MENU;
...
void SendPAGE_MAIN04(u8 initpw)
{
    ...
    communicate_T123_data[3]   = GetU16Low(scr);
	communicate_T123_data[4]   = GetU16High(scr);
	SendEnd();
...
void SendPAGE_MAIN06(u8 initpw)
{
    ...
    communicate_T123_data[8]   = ENABLEDIANHAO;//((D_SW||REVERSE_GEAR_SW) && (eng_speed > 0)/*&&(motor_torque > 0)*/);
