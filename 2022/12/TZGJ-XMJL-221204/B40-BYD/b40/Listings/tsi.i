...
static void save_odo()
{
    ...
    eeprom_sector_erase ((((((uint32_t)0x40000000)+0x40U*2)-((uint32_t)0x40000000))>>6), TRUE);
    eeprom_prog_Nwords ((((uint32_t)0x40000000)+0x40U*2), ((uint32_t *)(&tsb)), ((sizeof(TSB)+3)>>2));
    eeprom_sector_erase ((((((uint32_t)0x40000000)+0x40U*3)-((uint32_t)0x40000000))>>6), TRUE);
    eeprom_prog_Nwords ((((uint32_t)0x40000000)+0x40U*3), ((uint32_t *)(&tsb)), ((sizeof(TSB)+3)>>2));
}
...
int save_odo_pt (struct pt *pt)
{
    ...
    { char PT_YIELD_FLAG = 1; PT_YIELD_FLAG = PT_YIELD_FLAG; switch((pt)->lc) { case 0:;
    ...
    eeprom_sector_erase ((((((uint32_t)0x40000000)+0x40U*2)-((uint32_t)0x40000000))>>6), FALSE);
    { time = systick_ms + (25); do { (pt)->lc = 72; case 72:; if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };

    addr_temp = (((uint32_t)0x40000000)+0x40U*2);
    for (i = 0; i < ((sizeof(TSB)+3)>>2); i++)
    {
        eeprom_prog_word_nowait (addr_temp, ((uint32_t *)(&tsb))[i]);
        { time = systick_ms + (1); do { (pt)->lc = 78; case 78:; if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };
        addr_temp += 4;
    }

    eeprom_sector_erase ((((((uint32_t)0x40000000)+0x40U*3)-((uint32_t)0x40000000))>>6), FALSE);
    { time = systick_ms + (25); do { (pt)->lc = 83; case 83:; if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };

    addr_temp = (((uint32_t)0x40000000)+0x40U*3);
    for (i = 0; i < ((sizeof(TSB)+3)>>2); i++)
    {
        eeprom_prog_word_nowait (addr_temp, ((uint32_t *)(&tsb))[i]);
        { time = systick_ms + (1); do { (pt)->lc = 89; case 89:; if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };
        addr_temp += 4;
    }
...
{
    TSB  p2;
    eeprom_Nwords_get ((((uint32_t)0x40000000)+0x40U*2), (uint32_t *)&tsb, ((sizeof(TSB)+3)>>2));
    eeprom_Nwords_get ((((uint32_t)0x40000000)+0x40U*3), (uint32_t *)&p2, ((sizeof(TSB)+3)>>2));

    switch (check ((void *)&tsb, (sizeof(TSB))) | (check ((void *)&p2, (sizeof(TSB))) << 1))
    {
        case 0:
            reset_odo();
            save_odo();
...
__attribute__((used)) const init_fn_t __stan_init_init_odo __attribute__((section(".init_fn." "3"))) = init_odo;
