...
uint16_t avargeF(uint16_t data); uint16_t avargeF(uint16_t data) { static uint8_t count = 0; static uint8_t first = 0; static uint16_t buffer[(1 << 5)] = {0}; static uint32_t sum = 0; sum -= buffer[count & ((1 << 5) - 1)]; buffer[count & ((1 << 5) - 1)] = data; sum += data; count++; if (first < (1 << 5)) { first++; return sum / first; } else return sum >> 5; };
uint16_t avargeG(uint16_t data); uint16_t avargeG(uint16_t data) { static uint8_t count = 0; static uint8_t first = 0; static uint16_t buffer[(1 << 5)] = {0}; static uint32_t sum = 0; sum -= buffer[count & ((1 << 5) - 1)]; buffer[count & ((1 << 5) - 1)] = data; sum += data; count++; if (first < (1 << 5)) { first++; return sum / first; } else return sum >> 5; };
...
int protothread_adc (struct pt *pt)
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < 15; i++)
        {
            { time = systick_ms + (1); do { do { LC_LABEL62: ((pt)->lc) = &&LC_LABEL62; } while(0); if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };
            adc_start_convert (PoutS[i].adc_ch);
            { time = systick_ms + (1); do { do { LC_LABEL64: ((pt)->lc) = &&LC_LABEL64; } while(0); if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };
        ...
        { time = systick_ms + (1); do { do { LC_LABEL74: ((pt)->lc) = &&LC_LABEL74; } while(0); if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };
        adc_start_convert (22);
        { time = systick_ms + (1); do { do { LC_LABEL76: ((pt)->lc) = &&LC_LABEL76; } while(0); if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };

        ad = avargeF(adc_data_get());
        ...
        { time = systick_ms + (1); do { do { LC_LABEL94: ((pt)->lc) = &&LC_LABEL94; } while(0); if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };
        adc_start_convert (15);
        { time = systick_ms + (1); do { do { LC_LABEL96: ((pt)->lc) = &&LC_LABEL96; } while(0); if(!(((int)(systick_ms - time) >= 0))) { return 0; } } while(0); };

        ad = avargeG(adc_data_get());
        ...
        do { do { LC_LABEL119: ((pt)->lc) = &&LC_LABEL119; } while(0); if(!(((int) (systick_ms - time1) >= 0))) { return 0; } } while(0);
...
static void ain_init()
{
    (&pt1)->lc = 0;
				...
				gpio_bit_set(((((uint32_t)0x50000000) + (uint32_t)0xA0000) + (uint32_t)0x0038), ((uint32_t)(0x01UL<<((uint8_t)(7)))));


	
}
__attribute__((used)) const init_fn_t __stan_init_ain_init __attribute__((section(".init_fn." "4"))) = ain_init;
