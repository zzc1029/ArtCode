...
int save_param_pt (struct pt *pt)
{
    ...
    param.sum = sumx ((&param), sizeof (Param));

    eeprom_sector_erase (((((uint32_t)0x40000000)-((uint32_t)0x40000000))>>6), FALSE);
    do { (pt)->lc = 47; case 47:; if(!((is_eeprom_idle()))) { return 0; } } while(0);

    addr_temp = ((uint32_t)0x40000000);
    for (i = 0; i < ((sizeof(Param)+3)>>2); i++)
    {
        eeprom_prog_word_nowait (addr_temp, ((uint32_t *)(&param))[i]);
        do { (pt)->lc = 53; case 53:; if(!((is_eeprom_idle()))) { return 0; } } while(0);
        addr_temp += 4;
    }

    eeprom_sector_erase ((((((uint32_t)0x40000000)+0x40U)-((uint32_t)0x40000000))>>6), FALSE);
    do { (pt)->lc = 58; case 58:; if(!((is_eeprom_idle()))) { return 0; } } while(0);

    addr_temp = (((uint32_t)0x40000000)+0x40U);
    for (i = 0; i < ((sizeof(Param)+3)>>2); i++)
    {
        eeprom_prog_word_nowait (addr_temp, ((uint32_t *)(&param))[i]);
        do { (pt)->lc = 64; case 64:; if(!((is_eeprom_idle()))) { return 0; } } while(0);
        addr_temp += 4;
    }
...
{
    param.sum = sumx ((&param), sizeof (Param));
    eeprom_sector_erase (((((uint32_t)0x40000000)-((uint32_t)0x40000000))>>6), TRUE);
    eeprom_prog_Nwords (((uint32_t)0x40000000), ((uint32_t *)(&param)), ((sizeof(Param)+3)>>2));
    eeprom_sector_erase ((((((uint32_t)0x40000000)+0x40U)-((uint32_t)0x40000000))>>6), TRUE);
    eeprom_prog_Nwords ((((uint32_t)0x40000000)+0x40U), ((uint32_t *)(&param)), ((sizeof(Param)+3)>>2));
}

static void init_param()
{
    Param  p2;
    eeprom_Nwords_get (((uint32_t)0x40000000), (uint32_t *)&param, ((sizeof(Param)+3)>>2));
    eeprom_Nwords_get ((((uint32_t)0x40000000)+0x40U), (uint32_t *)&p2, ((sizeof(Param)+3)>>2));

    switch (check ((void *)&param, (sizeof(Param))) | (check ((void *)&p2, (sizeof(Param))) << 1))
    {
        case 0:
            reset_param();
            save_param();
...
__attribute__((used)) const init_fn_t __stan_init_init_param __attribute__((section(".init_fn." "3"))) = init_param;
