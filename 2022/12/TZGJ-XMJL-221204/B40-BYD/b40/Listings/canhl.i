...
uint32_t can_send_bytes(uint32_t id, uint8_t isext, uint8_t *data, uint8_t len)
{
		...
		if ((pfifo->max_len - pfifo->wr_index + pfifo->rd_index) >= 1)
    {
			uint32_t i;
...
int protothread_canhl_loop(struct pt *pt)
{
  { char PT_YIELD_FLAG = 1; PT_YIELD_FLAG = PT_YIELD_FLAG; do { if((pt)->lc != 0) { goto *(pt)->lc; } } while(0);
	...
	while(1)
	{
		...
		do { do { LC_LABEL181: ((pt)->lc) = &&LC_LABEL181; } while(0); if(!((pfifo->wr_index != pfifo->rd_index))) { return 0; } } while(0);				
		...
		while(retry ++ < 3)
		{
			...
			tx_timeout = systick_ms + 10;			
			do { do { LC_LABEL192: ((pt)->lc) = &&LC_LABEL192; } while(0); if(!(((int)(systick_ms - tx_timeout) >= 0)||((can_interrupt_flag_get() & ((uint32_t)(0x01UL<<((uint8_t)(1))))) != 0x00U))) { return 0; } } while(0);
...
void canhl_loop()
{
	...
	if((can_status_get() & ((uint32_t)(0x01UL<<((uint8_t)(7))))) != 0x00U)
	{
			can_config();
	}
}
