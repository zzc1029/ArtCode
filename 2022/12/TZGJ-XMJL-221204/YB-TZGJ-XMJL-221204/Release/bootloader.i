...
static CAN_SFRmap * can_handle = 0;
...
{
 uint8_t i;
 sendframecmd.m_FrameFormat = ((uint32_t)0);
 sendframecmd.m_RemoteTransmit = ((uint32_t)0);
 sendframecmd.m_DataLength = 8;
 sendframecmd.m_StandardID = 0x52E;
...
void BootloaderLoop()
{
 ...
 case BL_GetInfo:
  ...
  sendframecmd.m_Data[3] = 0x71;
  sendframecmd.m_Data[4] = 0x01;
...
{
 uint8_t i;
 if (RxMessage->m_FrameFormat == ((uint32_t)0))
 {
  if (RxMessage->m_StandardID == 0x526)
  {
   switch (RxMessage->m_Data[0])
   {
   case BLCMD_ToBootStatus:
    FLASH_Wipe_Configuration_RAM(((uint32_t)0),0x08000);
