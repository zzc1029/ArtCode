...
# 9 "../source/display.c" 2
# 1 "D:/ARTStanHome/Desktop/TZGJ-XMJL-221204/YB-TZGJ-XMJL-221204/driver/keyfifo.h" 1
# 12 "D:/ARTStanHome/Desktop/TZGJ-XMJL-221204/YB-TZGJ-XMJL-221204/driver/keyfifo.h"
...
SendPackFunc Main_Data_Sendfuncs[] =
{
 SendPAGE_MAIN04,
 SendPAGE_MAIN05,
 SendPAGE_MAIN06
...
void SendPAGE_TMS01(u8 initpw)
{
 ...
 Send_To_DP_Data[3] = time.hour;
 Send_To_DP_Data[4] = time.mins;
 Send_To_DP_Data[5] = time.second;
...
void SendPAGE_ABS01(u8 initpw)
{
 ...
 Send_To_DP_Data[3] = ABS_DTC[0];
 Send_To_DP_Data[4] = (ABS_DTC[0] >> 8);
 Send_To_DP_Data[5] = ((ABS_DTC[0] >> 21) & 0x07);
 Send_To_DP_Data[6] = 0;
 Send_To_DP_Data[7] = ((ABS_DTC[0] >> 16) & 0x1F);
...
void SendPAGE_ABS02(u8 initpw)
{
 ...
 Send_To_DP_Data[3] = ABS_DTC[1];
 Send_To_DP_Data[4] = (ABS_DTC[1] >> 8);
 Send_To_DP_Data[5] = ((ABS_DTC[1] >> 21) & 0x07);
 Send_To_DP_Data[6] = 0;
 Send_To_DP_Data[7] = ((ABS_DTC[1] >> 16) & 0x1F);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_ABS03(u8 initpw)
{
 ...
 Send_To_DP_Data[3] = ABS_DTC[2];
 Send_To_DP_Data[4] = (ABS_DTC[2] >> 8);
 Send_To_DP_Data[5] = ((ABS_DTC[2] >> 21) & 0x07);
 Send_To_DP_Data[6] = 0;
 Send_To_DP_Data[7] = ((ABS_DTC[2] >> 16) & 0x1F);
...
void SendPAGE_ABS04(u8 initpw)
{
 ...
 Send_To_DP_Data[3] = ABS_DTC[3];
 Send_To_DP_Data[4] = (ABS_DTC[3] >> 8);
 Send_To_DP_Data[5] = ((ABS_DTC[3] >> 21) & 0x07);
 Send_To_DP_Data[6] = 0;
 Send_To_DP_Data[7] = ((ABS_DTC[3] >> 16) & 0x1F);

    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
# 299 "../source/display.c"
...
void SendPAGE_0x29(u8 initpw)
{
 ...
 if((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE1] & 0xC0) == 0)
 {
  Send_To_DP_Data[3] = 0;
  temp = (((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE3])&0x0F) + ((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE3])>>4)*10);
  Send_To_DP_Data[4] = temp;
  temp = (((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE4])&0x0F) + ((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE4])>>4)*10);
  Send_To_DP_Data[5] = temp;
 }
 else if((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE1] & 0xC0) == 0x40)
 {
  temp = (((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE3])&0x0F) + ((CanData[Pcan1GetIndexFromID(0x18FFA898)][BYTE3])>>4)*10);
...
void SendPAGE_PEIDIAN07(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN08(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 4])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 4])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN09(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 8])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 8])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN0A(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 12])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 12])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN0B(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 16])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 16])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN0C(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 20])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 20])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN0D(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 24])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 24])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN0E(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 28])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 28])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN0F(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 32])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 32])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN10(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 36])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 36])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_PEIDIAN11(u8 initpw)
{
 ...
 for(i=0;i<4;i++)
  Send_To_DP_Data[3 + i*2] = (((PeiDianOutCur[i + 40])>>0)&0xFF);
 for(i=0;i<4;i++)
  Send_To_DP_Data[4 + i*2] = (((PeiDianOutCur[i + 40])>>8)&0xFF);
 for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
...
void SendPAGE_PEIDIAN14(u8 initpw)
{
 ...
 for(i=0;i<6;i++)
  Send_To_DP_Data[3 + i] = CanData[Pcan1GetIndexFromID(0x181F081F)][i];
 for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
...
void SendPAGE_PEIDIAN21(u8 initpw)
{
 ...
 for(i=0;i<8;i++)
  Send_To_DP_Data[3 + i] = CanData[Pcan1GetIndexFromID(0x181F061F)][i];
 for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}



u32 Menu_PEIDIANHE(u8 key, u8 initpw)
{
...
void PageMIEHUOMsg(u8 initpw, u8 max_box, u8 box)
{
    u8 i; u8 change = 0; static u8 data1save[11]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; for(i = 1; i < 12; i++) Send_To_DP_Data[i] = 0;;
 ...
 if(timeout[Pcan1GetIndexFromID(0x18FF56F6)] > 100)
...
u32 Menu_MEIHUO(u8 key, u8 initpw)
{
    static u8 flag = 0, changescreenflag = 0, fan = 2;
    u8 i;
    static u8 timecount;
...
void Send0x2301(u8 initpw)
{
 ...
 Send_To_DP_Data[0] = 0x23;
    ...
    Send_To_DP_Data[3] = taiya.yali[0];
    Send_To_DP_Data[4] = (((taiya.wendu[0])>>0)&0xFF);

    Send_To_DP_Data[5] = (((taiya.wendu[0])>>8)&0xFF);
 Send_To_DP_Data[6] = taiya.yali[1];

    Send_To_DP_Data[7] = (((taiya.wendu[1])>>0)&0xFF);
    Send_To_DP_Data[8] = (((taiya.wendu[1])>>8)&0xFF);

    Send_To_DP_Data[9] = taiya.yali[2];
...
void Send0x2302(u8 initpw)
{
 ...
 Send_To_DP_Data[0] = 0x23;
    ...
    Send_To_DP_Data[3] = (((taiya.wendu[2])>>0)&0xFF);
    Send_To_DP_Data[4] = (((taiya.wendu[2])>>8)&0xFF);

    Send_To_DP_Data[5] = taiya.yali[3];
 Send_To_DP_Data[6] = (((taiya.wendu[3])>>0)&0xFF);

    Send_To_DP_Data[7] = (((taiya.wendu[3])>>8)&0xFF);


    Send_To_DP_Data[8] = taiya.yali[4];

 Send_To_DP_Data[9] = (((taiya.wendu[4])>>0)&0xFF);

    Send_To_DP_Data[10] = (((taiya.wendu[4])>>8)&0xFF);
...
void Send0x2303(u8 initpw)
{
 ...
 Send_To_DP_Data[0] = 0x23;
    ...
    Send_To_DP_Data[3] = taiya.yali[5];

 Send_To_DP_Data[4] = (((taiya.wendu[5])>>0)&0xFF);

    Send_To_DP_Data[5] = (((taiya.wendu[5])>>8)&0xFF);
...
SendPackFunc PAGE_TAIYA_Sendfuncs[] =
{
 Send0x2301,
 Send0x2302,
 Send0x2303,
};
void PAGE_TAIYA_Send(u8 initpw) { u8 i=0; static u8 fan = 0; if(fan>= (sizeof(PAGE_TAIYA_Sendfuncs)/sizeof(SendPackFunc))) fan = 0; if(initpw) { for(i=0;i<(sizeof(PAGE_TAIYA_Sendfuncs)/sizeof(SendPackFunc));i++) (*(PAGE_TAIYA_Sendfuncs[i]))(initpw); } else (*(PAGE_TAIYA_Sendfuncs[fan]))(0); fan++; };
...
void InitDisplay()
{
 ...
 AddMenu(Menu_TMS, DIS_TMS_MENU);
    ...
    AddMenu(MenuDIS_TAIYA_MENU, DIS_TAIYA_MENU);
    AddMenu(Menu_EPB, DIS_EPB_MENU);

    AddMenu(Menu_RUNHUA, DIS_RUNHUA_MENU);

    AddMenu(Menu_PEIDIANHE, DIS_PEIDIANHE_MENU);
    AddMenu(Menu_MEIHUO, DIS_MIEHUO_MENU);
