...
{
 xGPIO_CAN();
 xInit_CAN(((CAN_SFRmap *) ((uint32_t)0x40001100)),CAN_BAUDRATE_125K,((uint32_t)0<<(1)));
...
uint32_t BCan_Standard_Tx_Bytes(uint16_t id,uint8_t * buffer,uint8_t len)
{
 ...
 CanSendBuff.buff[temp].Pcan1GetIndexFromID = id;
...
uint32_t BCanSendFifo(void)
{
 ...
 Candomsg.Pcan1GetIndexFromID = CanSendBuff.buff[temp].Pcan1GetIndexFromID;
...
void BCanSendTo(void)
{
   ...
   CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40001100)),Candomsg.Pcan1GetIndexFromID,Candomsg.data,Candomsg.len,((uint32_t)0),Candomsg.type);
     }
     break;
     case CANDELAYING:
...
volatile uint8_t BCanData[(sizeof(BCanIdTable)/sizeof(BCanIdTable[0]))][8];


uint16_t BcanGetIndexFromID(uint32_t id)
{
 uint16_t i;
 for(i=0;i<((sizeof(BCanIdTable)/sizeof(BCanIdTable[0])));i++)
 {
  if(BCanIdTable[i] == id)
   return i;
 }
 return ((sizeof(BCanIdTable)/sizeof(BCanIdTable[0])));
}
...
{
 if(index < ((sizeof(BCanIdTable)/sizeof(BCanIdTable[0]))))
 {
   ...
   case 0x526:
    switch (data[0])
    {
     case BLCMD_ToBootStatus:
       { INT_All_Enable (FALSE); FLASH_Wipe_Configuration_RAM (((uint32_t)0),0x8000); INT_All_Enable (TRUE);};
