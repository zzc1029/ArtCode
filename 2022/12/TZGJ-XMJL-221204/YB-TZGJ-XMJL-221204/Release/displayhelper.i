...
# 10 "../source/displayhelper.c" 2
 ...
 void ChangeToSubPageCache(uint8_t menu, uint8_t subpage)
 {
  ...
  switch(menu)
  {
   ...
   case DIS_PEIDIANHE_MENU:
            PageInfo[3].bit.bs0 = subpage;
            break;
   case DIS_MIEHUO_MENU:
       PageInfo[3].bit.bs1 = subpage;
    break;
         case SET_JMP_PAGE:
            choose_menu = subpage;
...
# 165 "../source/displayhelper.c"
 static u32 MainMesg(u8 key, u8 initpw)
 {
  static u8 changescreenflag = 0,lop = 0;
     static u8 flagx = 0;
  ...
  if(changescreenflag)
    {
        changescreenflag = 0;
        ChangeToSubPage(DIS_MAIN, flagx);
    }
# 202 "../source/displayhelper.c"
  switch(key)
  {
   case IN_KEY1_SP:
  ...
  case IN_KEY2_SP:
            flagx++;
            flagx = (flagx >= MAINMax) ? MAIN : flagx;
            changescreenflag = 1;
            break;
        case IN_KEY3_SP:
            flagx = (flagx == MAIN) ? MAINMax : flagx;
            flagx--;
            changescreenflag = 1;
            break;
     case IN_KEY4_CP:
...
void Art_Meter_4(u8 initpw)
{
 ...
 Send_To_DP_Data[3] = 1;
# 668 "../source/displayhelper.c"
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
# 692 "../source/displayhelper.c"
...
void SendESSetDataToGPU()
{
 ...
 static u8 ChangeOilAlarm(u8 key, u8 initpw)
 {
  ...
  {
   initflag = 0;
    Set_Oil_Alarm_last = Set_Oil_Alarm;
  ...
  if(kehu_mi)
  {
   ...
   {
    Set_Oil_Alarm = Set_Oil_Alarm_last;
    Art.bit.b39 = 1;
   }
   else
    Art.bit.b39 = 0;
  ...
  switch(flag)
  {
   case 0:
    {
     ...
     else
     {
      ...
      if(kehu_mi)
                     ChangeToSubPage(DIS_SYSTEM_SET_MENU, PAGE_KeHuSET_MILESET);
     else
   ...
   case 1:
    switch(flag1)
    {
     ...
     flag1 = (flag1 >= EncryptSystemSetLast ? kehu_mi : flag1);
...
# 3150 "../source/displayhelper.c"
 ...
 void Send_Symbo2_ToDis(u8 initpw)
 {
  u8 i; u8 change = 0; static u8 data1save[11]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; for(i = 1; i < 12; i++) Send_To_DP_Data[i] = 0;;
  ...
  Send_To_DP_Data[3] = led[8].byte;
  Send_To_DP_Data[4] = led[9].byte;
  Send_To_DP_Data[5] = led[10].byte;

  for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
 }

 void Send_Symbo_trip(u8 initpw)
 {
  u8 i; u8 change = 0; static u8 data1save[11]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; for(i = 1; i < 12; i++) Send_To_DP_Data[i] = 0;;
  ...
  Send_To_DP_Data[3] = Veh_trip;
  Send_To_DP_Data[4] = Veh_trip >> 8;
  Send_To_DP_Data[5] = Veh_trip >> 16;
     Send_To_DP_Data[6] = Veh_trip >> 24;

     if(port_a.bit.b1)
    Send_To_DP_Data[8] = 7;
  else
    Send_To_DP_Data[8] = (CanData[Pcan1GetIndexFromID(0x18FFEA97)][1] >> 3) & 0x07;
 ...
 void Send_Guide_ToDis2(u8 initpw)
 {
  ...
  Send_To_DP_Data[3] = 0;
     Send_To_DP_Data[4] = 0;
     Send_To_DP_Data[5] = 0;
     Send_To_DP_Data[6] = veh_soc;

     temp = ((((uint16_t)CanData[Pcan1GetIndexFromID(0x18F13DF3)][BYTE3+1])<<8)+CanData[Pcan1GetIndexFromID(0x18F13DF3)][BYTE3]) / 10;

     Send_To_DP_Data[7] = ((uint8_t)(((temp)>>0)&0xFF));
     Send_To_DP_Data[8] = ((uint8_t)(((temp)>>8)&0xFF));
     temp = ((((uint16_t)CanData[Pcan1GetIndexFromID(0x18F13DF3)][BYTE1+1])<<8)+CanData[Pcan1GetIndexFromID(0x18F13DF3)][BYTE1]) / 10;
     Send_To_DP_Data[9] = (((temp)>>0)&0xFF);
     Send_To_DP_Data[10] = (((temp)>>8)&0xFF);
 ...
 {
  if((port_a.bit.b7 && (Art.bit.b13)) || port_a.bit.b1)
  {
   uint8_t i;
 ...
 void roll_symbol(void)
  {
   ...
   t1 = ((u32)Fault[3].byte << 24)+((u32)Fault[2].byte << 16)+
     ((u16)Fault[1].byte << 8)+Fault[0].byte ;
   t2 = Fault[4].byte;
  restart:
