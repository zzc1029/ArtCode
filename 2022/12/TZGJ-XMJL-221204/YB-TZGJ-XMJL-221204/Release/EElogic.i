...
# 9 "../driver/EElogic.c" 2
# 1 "../driver/ZD240Cx.h" 1
# 10 "../driver/EElogic.c" 2
# 1 "../driver/EElogic.h" 1
# 11 "../driver/EElogic.c" 2
...
void WriteMainOdo(void)
{
  ...
  ZD_write_bytes(16*backupmilepage,eeodo.data8,16);




  DelayKong(400,400);
  ZD_write_bytes(16*mainmilepage,eeodo.data8,16);
  DelayKong(400,400);
...
{
   u32 miles;
   ZD_read_bytes(16*mainmilepage,eeodo.data8,16);
   miles = ((((uint32_t)eeodo.data8[0 + 3])<<24)+(((uint32_t)eeodo.data8[0 + 2])<<16)+(((uint32_t)eeodo.data8[0 + 1])<<8)+eeodo.data8[0]);
   if(miles == 0XFFFFFFFF)
   {
      Veh_odo = 0;
   Veh_trip = 0;
   ...
   else
   {
      ...
      {
         Veh_odo = ((((uint32_t)eeodo.data8[0 + 3])<<24)+(((uint32_t)eeodo.data8[0 + 2])<<16)+(((uint32_t)eeodo.data8[0 + 1])<<8)+eeodo.data8[0]);
   Veh_trip = ((((uint16_t)eeodo.data8[4 +1])<<8)+eeodo.data8[4]);
   }
   else
   {
        ZD_read_bytes(16*backupmilepage,eeodo.data8,16);
  DelayKong(400,400);
  if(eeodo.Data.checksum == EEOdoDataPlus(eeodo.data8,sizeof(eeodo.data8)))
  {
           Veh_odo = ((((uint32_t)eeodo.data8[0 + 3])<<24)+(((uint32_t)eeodo.data8[0 + 2])<<16)+(((uint32_t)eeodo.data8[0 + 1])<<8)+eeodo.data8[0]) + 1;
     Veh_trip = ((((uint16_t)eeodo.data8[4 +1])<<8)+eeodo.data8[4]);
     eeodo.Data.odo = Veh_odo;
        eeodo.Data.trip = Veh_trip;
     ZD_write_bytes(16*mainmilepage,eeodo.data8,16);
...
{
    Art.bit.b3 = eepara.Data.Switch.b0;
 Art.bit.b4 = eepara.Data.Switch.b1;
 Art.bit.b5 = eepara.Data.Switch.b2;
 Art.bit.b6 = eepara.Data.Switch.b3;
 Art.bit.b7 = eepara.Data.Switch.b4;
    Art.bit.b2 = eepara.Data.Switch.b5;
...
{
 eepara.Data.Switch.b0 = Art.bit.b3;
 eepara.Data.Switch.b1 = Art.bit.b4;
 eepara.Data.Switch.b2 = Art.bit.b5;
 eepara.Data.Switch.b3 = Art.bit.b6;
 eepara.Data.Switch.b4 = Art.bit.b7;
 eepara.Data.Switch.b5 = Art.bit.b2;
...
{
   u8 i;
   ZD_read_bytes(16*mainparal,eepara.data8,16*2);
   if(eepara.Data.backlight == 0xff)
   {
      CopyData(eepara.data8,EEConstInit.data8,sizeof(EEConstInit));
   eepara.Data.checksum = EEDataPlus(eepara.data8,sizeof(eepara.data8));
   EEGiveParaToData();
   ZD_write_bytes(16*backupparal,eepara.data8,16);
   DelayKong(300,300);
   ZD_write_bytes(16*backupparah,eepara.data8 + 16,16);
   DelayKong(300,300);
   ZD_write_bytes(16*mainparal,eepara.data8,16);
   DelayKong(300,300);
   ZD_write_bytes(16*mainparah,eepara.data8 + 16,16);
   DelayKong(300,300);
   }
   else
   {
   ...
   {
        ZD_read_bytes(16*backupparal,eepara.data8,16*2);
  if(eepara.Data.checksum == EEDataPlus(eepara.data8,sizeof(eepara.data8)))
  {
           EEGiveParaToData();
     ZD_write_bytes(16*mainparal,eepara.data8,16);
...
void WriteEE(void)
{
  ...
  switch(step)
  {
    ...
    else if(EEflag.bit.b1)
    {
       EESaveDataToMeter();
     step = EEBACKPARAL;
    }
    else if(EEflag.bit.b2)
    {
       checksum = EEDataPlus(CanSetEEByte.data8,sizeof(CanSetEEByte.data8));
                ...
                {
                   if(CanSetEEByte.Data.cmd == 1)
                   {
                      ZD_write_add(CanSetEEByte.Data.addr,CanSetEEByte.Data.setdata);
       step = EECANSETBACK;
                   }
       else if(CanSetEEByte.Data.cmd == 2)
       {
       CanSetEEByte.Data.resv[0] = ZD_read_add(CanSetEEByte.Data.addr);
       step = EECANSETBACK;
       }
       else
       {
 ...
 case EECANSETBACK:
   ...
   {
            CanSetEEByte.Data.resv[1] = ZD_read_add(CanSetEEByte.Data.addr);
