...
static void xINT_BCAN(CAN_SFRmap* CANx)
{
 ...
 CAN_Set_INT_Enable(CANx,((uint32_t)1<<0)|((uint32_t)1<<7),TRUE);
...
void InitPCan2Send(void)
{
    Bcan_fifo_init(&PCan2SendBuff,PCan2Sendbuffx,((1<<(6))));
...
void PCan2SendTo(void)
{
   ...
   BcanDelay = 2;
   if(PCan2domsg.type == ((uint32_t)0))
    CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40001980)),PCan2domsg.ID,PCan2domsg.data,PCan2domsg.len,((uint32_t)0),((uint32_t)0));
   else
    CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40001980)),PCan2domsg.ID,PCan2domsg.data,PCan2domsg.len,((uint32_t)0),((uint32_t)1));
     }
     break;
     case CANDELAYING:
     if((BcanDelay == 0) || (CAN_Get_Transmit_Status(((CAN_SFRmap *) ((uint32_t)0x40001980)),((uint32_t)((uint32_t)1<<((18)))))))
     {
     PCan2domsg.sended = CANSENDED;
   BcanDelay = 0;
     }
...
volatile uint8_t PCan2Data[(sizeof(PCan2IdTable)/sizeof(PCan2IdTable[0]))][8];


uint16_t Pcan2GetIndexFromID(uint32_t id)
{
 uint16_t i;
 for(i=0;i<((sizeof(PCan2IdTable)/sizeof(PCan2IdTable[0])));i++)
 {
  if(PCan2IdTable[i] == id)
   return i;
 }
 return ((sizeof(PCan2IdTable)/sizeof(PCan2IdTable[0])));
}
...
u32 Pcan2GetIDFromIndex(u16 index)
{
 if(index<((sizeof(PCan2IdTable)/sizeof(PCan2IdTable[0]))))
 {
  return PCan2IdTable[index];
 }
 else
 {
  return 0;
...
void Can3_Rx_Isr(CAN_MessageTypeDef CAN_MessageStructrue)
{
   ...
   index = Pcan2GetIndexFromID(id);
   if(index < ((sizeof(PCan2IdTable)/sizeof(PCan2IdTable[0]))))
   {

    GeCantData(CAN_MessageStructrue,PCan2Data[index]);
