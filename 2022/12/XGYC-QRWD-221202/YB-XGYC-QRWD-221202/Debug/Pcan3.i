...
static void xGPIO_Pcan3(void)
{
 ...
 GPIO_Pin_RMP_Config(((GPIO_SFRmap *) ((uint32_t)0x50000100)),((uint32_t)0x5),((uint8_t)0x09));
 GPIO_Pin_RMP_Config(((GPIO_SFRmap *) ((uint32_t)0x50000100)),((uint32_t)0x6),((uint8_t)0x09));
}
# 38 "../driver/Pcan3.c"
...
void PCan3SendTo(void)
{
   ...
   Pcan3Delay = 2;
   if(PCan3domsg.type == ((uint32_t)0))
    CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40002880)),PCan3domsg.ID,PCan3domsg.data,PCan3domsg.len,((uint32_t)0),((uint32_t)0));
   else
    CAN_Transmit_DATA(((CAN_SFRmap *) ((uint32_t)0x40002880)),PCan3domsg.ID,PCan3domsg.data,PCan3domsg.len,((uint32_t)0),((uint32_t)1));
     }
     break;
     case CANDELAYING:
     if((Pcan3Delay == 0) || (CAN_Get_Transmit_Status(((CAN_SFRmap *) ((uint32_t)0x40002880)),((uint32_t)((uint32_t)1<<((18)))))))
     {
     PCan3domsg.sended = CANSENDED;
   Pcan3Delay = 0;
     }
...
volatile uint8_t PCan3Data[(sizeof(PCan3IdTable)/sizeof(PCan3IdTable[0]))][8];


uint16_t PCAN3GetIndexFromID(uint32_t id)
{
 uint16_t i;
 for(i=0;i<((sizeof(PCan3IdTable)/sizeof(PCan3IdTable[0])));i++)
 {
  if(PCan3IdTable[i] == id)
   return i;
 }
 return ((sizeof(PCan3IdTable)/sizeof(PCan3IdTable[0])));
}
...
u32 Pcan3GetIDFromIndex(u16 index)
{
 if(index<((sizeof(PCan3IdTable)/sizeof(PCan3IdTable[0]))))
 {
  return PCan3IdTable[index];
 }
 else
 {
  return 0;
...
void Can4_Rx_Isr(CAN_MessageTypeDef CAN_MessageStructrue)
{
   ...
   index = PCAN3GetIndexFromID(id);
   if(index < ((sizeof(PCan3IdTable)/sizeof(PCan3IdTable[0]))))
   {

    GeCantData(CAN_MessageStructrue,PCan3Data[index]);
