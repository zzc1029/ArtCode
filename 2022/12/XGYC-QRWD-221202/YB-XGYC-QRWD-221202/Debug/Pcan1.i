 ...
 switch(DCandomsg.sended)
 {
   case CANSENDED:
    ...
    if(DCandomsg.type == ((uint32_t)0))
     ...
     break;
   case CANDELAYING:
     if((DcanDelay == 0) || (CAN_Get_Transmit_Status(((CAN_SFRmap *) ((uint32_t)0x40001900)),((uint32_t)((uint32_t)1<<((18)))))))
     {
    DCandomsg.sended = CANSENDED;
    DcanDelay = 0;
     }
...
volatile uint8_t DCanData[(sizeof(DCanIdTable)/sizeof(DCanIdTable[0]))][8];


 uint16_t DGetIndexFromID(uint32_t id)
 {
  uint16_t i;
  for(i=0;i<((sizeof(DCanIdTable)/sizeof(DCanIdTable[0])));i++)
  {
   if(DCanIdTable[i] == id)
    return i;
  }
  return ((sizeof(DCanIdTable)/sizeof(DCanIdTable[0])));
 }
 ...
 {
  if(index<((sizeof(DCanIdTable)/sizeof(DCanIdTable[0]))))
  {
