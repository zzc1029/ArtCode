...
void Send0x2E01(u8 initpw)
{
	...
	communicate_T123_data[4]   = GetU16Low(vol);
	communicate_T123_data[5]   = GetU16High(vol);
	communicate_T123_data[6]   = GetU16Low(cur);
	communicate_T123_data[7]   = GetU16High(cur);
	communicate_T123_data[8]   = BMS.imono_max_temp_2;
	communicate_T123_data[9]   = GetU16Low(BMS.imono_max_voltage);//Motor_Temp;
	communicate_T123_data[10]  = GetU16High(BMS.imono_max_voltage);
...
void Send0x2E02(u8 initpw)
{
	...
	communicate_T123_data[3]   = GetU16Low(BMS.imono_min_voltage);
	communicate_T123_data[4]   = GetU16High(BMS.imono_min_voltage);
	communicate_T123_data[5]   = BMS.imono_min_temp;
	communicate_T123_data[6]   = GetU16Low(BMS.itotal_anode_riso); 
	communicate_T123_data[7]   = GetU16High(BMS.itotal_anode_riso);
	communicate_T123_data[8]   = GetU16Low(BMS.itotal_cathode_riso); 
	communicate_T123_data[9]  = GetU16High(BMS.itotal_cathode_riso);
	communicate_T123_data[10]  = SYMBOL_POSITION10|(SYMBOL_POSITION11 << 1);
...
void roll_alarm(void)
{
	...
	if(luntai_count[0] >= 6)
		luntai_count[0] = 0;
	for(;luntai_count[0]<6;luntai_count[0]++)
	{
		if((TIRE.fault_2[luntai_count[0]] & 0xE0) == 0x40)
		{
			TIRE.baojing_count[0] |= (1 << luntai_count[0]);
			TIRE.alarm_num[0] = luntai_count[0];
			luntai_count[0]++;
			break;
		}
		else
	...
	TIP_0XC3 = (TIRE.baojing_count[0] > 0);
	if(TIP_0XC3 == 0)
		TIRE.alarm_num[0] = 0;
	...
	if(luntai_count[1] >= 6)
		luntai_count[1] = 0;
	for(;luntai_count[1]<6;luntai_count[1]++)
	{
		if((TIRE.fault_2[luntai_count[1]] & 0xE0) == 0x20)
		{
			TIRE.baojing_count[1] |= (1 << luntai_count[1]);
			TIRE.alarm_num[1] = luntai_count[1]++;
			break;
		}
		else
		{
	...
	TIP_0XC4 = (TIRE.baojing_count[1] > 0);
	if(TIP_0XC4 == 0)
		TIRE.alarm_num[1] = 0;
	...
	if(luntai_count[2] >= 6)
		luntai_count[2] = 0;
	for(;luntai_count[2]<6;luntai_count[2]++)
	{
		if((TIRE.fault_1[luntai_count[2]] & 0xC0) == 0x40)
		{
			TIRE.baojing_count[2] |= (1 << luntai_count[2]);
			TIRE.alarm_num[2] = luntai_count[2]++;
			break;
		}
		else
		{
	...
	TIP_0XC5 = (TIRE.baojing_count[2] > 0);
	if(TIP_0XC5 == 0)
		TIRE.alarm_num[2] = 0;
	//锟斤拷胎锟斤拷锟斤拷锟斤拷锟斤拷失
	if(luntai_count[3] >= 6)
		luntai_count[3] = 0;
	for(;luntai_count[3]<6;luntai_count[3]++)
	{
		if((TIRE.fault_1[luntai_count[3]] & 0x30) == 0x10)
		{
			TIRE.baojing_count[3] |= (1 << luntai_count[3]);
			TIRE.alarm_num[3] = luntai_count[3]++;
			break;
		}
		else
		{
	...
	TIP_0XC6 = (TIRE.baojing_count[3] > 0);
	if(TIP_0XC6 == 0)
		TIRE.alarm_num[3] = 0;
	//锟斤拷胎锟斤拷漏
	if(luntai_count[4] >= 6)
		luntai_count[4] = 0;
	for(;luntai_count[4]<6;luntai_count[4]++)
	{
		if((TIRE.fault_1[luntai_count[4]] & 0x0C) == 0x04)
		{
			TIRE.baojing_count[4] |= (1 << luntai_count[4]);
			TIRE.alarm_num[4] = luntai_count[4]++;
			break;
		}
		else
		{
	...
	TIP_0XC7 = (TIRE.baojing_count[4] > 0);
	if(TIP_0XC7 == 0)
		TIRE.alarm_num[4] = 0;
	#endif
	//*********************************/
...
void SendPAGE_MAIN05(u8 initpw)
{
    ...
    communicate_T123_data[6]   = GetU16Low (SystemCode);
    communicate_T123_data[7]   = GetU16High (SystemCode);
...
void SendPAGE_MAIN06(u8 initpw)
{
    ...
    communicate_T123_data[3]   = GetU16Low (shunshigonglv);
    communicate_T123_data[4]   = GetU16High (shunshigonglv);
