...
void SendPAGE_0x23_1(u8 initpw)
{
	...
	Send_To_DP_Data[4] = GetU16Low(taiya.temp[0]);
	Send_To_DP_Data[5] = GetU16High(taiya.temp[0]);
	Send_To_DP_Data[6] = taiya.press[1];
	Send_To_DP_Data[7] = GetU16Low(taiya.temp[1]);
	Send_To_DP_Data[8] = GetU16High(taiya.temp[1]);
...
void SendPAGE_0x23_2(u8 initpw)
{
	...
	Send_To_DP_Data[3] = GetU16Low(taiya.temp[2]);
	Send_To_DP_Data[4] = GetU16High(taiya.temp[2]);
	Send_To_DP_Data[5] = taiya.press[3];
	Send_To_DP_Data[6] = GetU16Low(taiya.temp[3]);
	Send_To_DP_Data[7] = GetU16High(taiya.temp[3]);
	Send_To_DP_Data[8] = taiya.press[4];
	Send_To_DP_Data[9] = GetU16Low(taiya.temp[4]);
	Send_To_DP_Data[10] = GetU16High(taiya.temp[4]);
    SendEnd();
}
void SendPAGE_0x23_3(u8 initpw)
{
	...
	Send_To_DP_Data[4] = GetU16Low(taiya.temp[5]);
	Send_To_DP_Data[5] = GetU16High(taiya.temp[5]);
...
void InitDisplay()
{
    ...
    //AddMenu(Menu_ABS, DIS_ABS_MENU);//3 电空调
    ...
    AddMenu(Menu_TAIYA, DIS_TAIYA_MENU);//3 可行驶里程和功率
