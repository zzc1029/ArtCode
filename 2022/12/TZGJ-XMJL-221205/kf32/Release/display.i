...
# 1 "D:/ARTStanHome/Desktop/TZGJ-XMJL-221205/kf32/driver/keyfifo.h" 1
# 12 "D:/ARTStanHome/Desktop/TZGJ-XMJL-221205/kf32/driver/keyfifo.h"
...
void SendPAGE_0x23_1(u8 initpw)
{
 u8 temp;
 u8 i; u8 change = 0; static u8 data1save[11]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; for(i = 1; i < 12; i++) Send_To_DP_Data[i] = 0;;
 ...
 Send_To_DP_Data[3] = taiya.press[0];
 Send_To_DP_Data[4] = (((taiya.temp[0])>>0)&0xFF);
 Send_To_DP_Data[5] = (((taiya.temp[0])>>8)&0xFF);
 Send_To_DP_Data[6] = taiya.press[1];
 Send_To_DP_Data[7] = (((taiya.temp[1])>>0)&0xFF);
 Send_To_DP_Data[8] = (((taiya.temp[1])>>8)&0xFF);
 Send_To_DP_Data[9] = taiya.press[2];
 Send_To_DP_Data[10] = 0;
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_0x23_2(u8 initpw)
{
 u8 temp;
 u8 i; u8 change = 0; static u8 data1save[11]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; for(i = 1; i < 12; i++) Send_To_DP_Data[i] = 0;;
    Send_To_DP_Data[0] = 0x23;
 ...
 Send_To_DP_Data[3] = (((taiya.temp[2])>>0)&0xFF);
 Send_To_DP_Data[4] = (((taiya.temp[2])>>8)&0xFF);
 Send_To_DP_Data[5] = taiya.press[3];
 Send_To_DP_Data[6] = (((taiya.temp[3])>>0)&0xFF);
 Send_To_DP_Data[7] = (((taiya.temp[3])>>8)&0xFF);
 Send_To_DP_Data[8] = taiya.press[4];
 Send_To_DP_Data[9] = (((taiya.temp[4])>>0)&0xFF);
 Send_To_DP_Data[10] = (((taiya.temp[4])>>8)&0xFF);
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
void SendPAGE_0x23_3(u8 initpw)
{
 u8 temp;
 u8 i; u8 change = 0; static u8 data1save[11]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; for(i = 1; i < 12; i++) Send_To_DP_Data[i] = 0;;
    Send_To_DP_Data[0] = 0x23;
 ...
 Send_To_DP_Data[3] = taiya.press[5];
 Send_To_DP_Data[4] = (((taiya.temp[5])>>0)&0xFF);
 Send_To_DP_Data[5] = (((taiya.temp[5])>>8)&0xFF);

    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}

static u32 Menu_TAIYA(u8 key, u8 initpw)
{
 if(initpw)
 {
  SendKeyToGPU(CMD_DIS_PAGE);
...
void InitDisplay()
{
 ...
 AddMenu(Menu_TMS, DIS_TMS_MENU);
    ...
    AddMenu(Menu_TAIYA, DIS_TAIYA_MENU);
...
{
# 965 "../source/display.c"
    if((alarm_count >= 0xDB) && (alarm_count <= 0xE9))
  return Meihuo[alarm_count - 0xDB].alarm_level;
 else
  return 0;
}
