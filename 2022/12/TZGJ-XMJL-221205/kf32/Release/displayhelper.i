...
void Art_Meter_4(u8 initpw)
{
 ...
 Send_To_DP_Data[3] = 1;
# 667 "../source/displayhelper.c"
    for(i=3; i<11; i++) { if(data1save[i] != Send_To_DP_Data[i]) { data1save[i] = Send_To_DP_Data[i]; change ++; } } if((change>0) ||(initpw)) { fifo_in(&SendBuff); };
}
# 691 "../source/displayhelper.c"
 ...
 void roll_alarm_Task(void)
 {
  if((port_a.bit.b7 && (Art.bit.b13)) || port_a.bit.b1)
  {
...

   {
    ...
    if((errcode > 0) && (Send_To_DP_Data[3] >=0X83) && (Send_To_DP_Data[3] <=0X9A))
             {
                Send_To_DP_Data[4] = CanData[Pcan1GetIndexFromID(0x18FB0CA7)][BYTE1];
             }
   }
